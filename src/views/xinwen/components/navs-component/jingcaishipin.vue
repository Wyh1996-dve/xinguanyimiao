 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2021-05-17 14:48:02
-->
<template>
  <div id="jingcaishipin">
    ...
    infinite-scroll-distance="10">
      <div id="cc" v-for="(item,i) in res" :key="i" style="font-size:16px;margin-top:10px;">
        <p style="font-weight:500;">{{item.username}}</p>
        <p>{{item.pl}}</p>
        <p>{{item.time}}</p>
      </div>
    </div>
  ...
  methods:{
    ...
    pinlun(){
        ...
        if(this.value==''){
                  ...
                  this.axios.post('/pl', qs.stringify({id:id,username:'pxy',pinlun:this.value,time:mytime})).then(res=>{
